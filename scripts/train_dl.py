...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--feature-type", default="logmel", choices=["logmel", "cwt"])
    p.add_argument(
        "--model-type",
        default="hybrid_spec_transformer",
    ...
    p.add_argument("override", nargs="*", help="YAML config overrides, e.g. epochs=30 lr=5e-4")
    return p
...
def main() -> None:
    ...
    cfg = TrainConfig(
        **{
            ...
            if hasattr(TrainConfig, k) or k in TrainConfig.__dataclass_fields__
        }
    ...
    if args.exclude_runs:
        ...
        logger.info(
            "Excluded %d files from excluded runs %s", before - len(file_df), args.exclude_runs
        )
    ...
    logger.info(
        "Starting repeated experiment (%s, %s, %s)", cfg.task, cfg.feature_type, cfg.model_type
    )
