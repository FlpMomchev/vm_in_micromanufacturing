...
Usage::
    ...
    vm-split single --input <file> --out-dir <dir> --segments-per-file 49
...
def _run_single(args: argparse.Namespace) -> None:
    ...
    doe_df = load_doe(args.doe_xlsx, sheet_name=args.doe_sheet) if args.doe_xlsx else None
    if doe_df is None:
        doe_df = pd.DataFrame(
            {
...
def _resolve_batch_configs(args: argparse.Namespace) -> list[tuple[str, dict]]:
    ...
    if missing:
        raise ValueError(f"Without --preset, must provide: {missing}")
    return [
        (
            "custom",
            {
